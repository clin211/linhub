...
// Diff 返回 base 中不包含在另一个 exclude 切片中的元素组成的切片。
...
// Include 返回 include 中同时存在于另一个 base 切片中的元素组成的切片。
...
// ContainsEqualFold 在给定切片 'slice' 在 Unicode 大小写折叠下包含字符串 's' 时返回 true。
