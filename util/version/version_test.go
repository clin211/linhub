...
func TestSemanticVersions(t *testing.T) {
	tests := []testItem{
		// 这里列出的是 semver 2.0 规范中出现的每个版本字符串，
		// 除特别说明外，均按严格递增的顺序排列。
		{version: "0.1.0"},
		{version: "1.0.0-0.3.7"},
		{version: "1.0.0-alpha"},
...
func TestGenericVersions(t *testing.T) {
	tests := []testItem{
		// 这里包含了 TestSemanticVersions 中的所有字符串，加上
		// TestBadSemanticVersions 中应该能作为通用版本被解析的部分，
		// 以及一些额外的字符串。
