...
// embeddedLogger 是一个类型别名，这样在嵌入时 embedded.Logger 类型
// 就不会与 recorder 的 Logger 方法发生冲突.
type embeddedLogger = embedded.Logger //nolint:unused  // 下方使用了.
...
// recorder 记录所有它被要求 emit 的 [log.Record].
type recorder struct {
	embedded.LoggerProvider
	embeddedLogger //nolint:unused  // 用于嵌入 embedded.Logger.

	// Records 是已发出（emit）的所有记录.
	Records []log.Record

	// Scope 是 recorder 在 Logger 被调用时所接收到的 Logger 作用域.
	Scope scope

	// MinSeverity 是 recorder 在 Enabled 被调用时返回 true 的最低
	// 严重级别（除非设置了 enableKey）.
...
// testCase 表示一次对待测 slog handler 的完整 setup/run/check 流程.
// 基于 "testing/slogtest" (1.22.1) 中的 testCase.
type testCase struct {
	// 子测试名称.
	name string
	// 若 explanation 非空，则用于说明被违反的约束.
	explanation string
	// f 使用其参数 logger 执行单次日志事件.
	// 为了让 mkdescs.sh 能生成正确的描述，
	// f 的函数体必须出现在单独一行上，且该行第一个
	// 非空白字符必须是 "l.".
	f func(*slog.Logger)
	// 如果 mod 不为 nil，则会在 Record 由 Logger 生成、传递给 Handler
	// 之前调用 mod 来修改该 Record.
	mod func(*slog.Record)
	// checks 是对结果运行的检查列表. 每一项都是若干 check 组成的切片，
	// 它们会针对对应发出的记录依次进行检查.
	checks [][]check
	// options 会传递给为该测试用例构造的 Handler.
	options []Option
}

// 从 slogtest (1.22.1) 拷贝而来.
	...
	cases := []testCase{
		{
			...
			f: func(l *slog.Logger) {
				l.Info(
					...
					// KindGroup 和 KindLogValuer 留给 slogtest.TestHandler 处理.
				)
			},
			checks: [][]check{{
				hasKey(slog.TimeKey),
